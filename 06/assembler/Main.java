...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        String fullFilePath = args[0]; //must be valid full path to file.
        Parser parser = new Parser(fullFilePath);
        Code code = new Code();
        ...
        String fileName = parser.getFilePath().replace("asm", "hack");
        try {
            File fout = new File(fileName);
            FileOutputStream fos = new FileOutputStream(fout);
            BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(fos));
            parser = new Parser(fullFilePath);
            while (parser.hasMoreCommands()) {
                parser.advance();
                if (parser.commandType().equals("A_COMMAND")) {
                    ...
                    try {
                        ...
                        currentCommand = Integer.toBinaryString(address);
                        ...
                        currentCommand = "0000000000000000".substring(currentCommand.length()) + currentCommand;
